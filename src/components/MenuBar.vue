<template>
  <div class="menu-bar">
    <transition name="silde-up">
      <div class="menu-wrapper" :class="{'hide-box-shadow': ifSettingShow || !ifTitleAddMenuShow}" v-show="ifTitleAddMenuShow">
        <div class="iconfont-wrapper">
          <span class="iconfont icon" @click="showSetting(0)">&#xe890;</span>
        </div>
        <div class="iconfont-wrapper">
          <span class="iconfont icon" @click="showSetting(1)">&#xe672;</span>
        </div>
        <div class="iconfont-wrapper">
          <span class="iconfont icon" @click="showSetting(2)">&#xe60a;</span>
        </div>
        <div class="iconfont-wrapper">
    ...
    <transition name="silde-up">
      <div class="setting-wrapper" v-show="ifSettingShow">
        <div class="setting-progress" v-if="showTag === 1">
        ...
        <div class="setting-theme" v-else-if="showTag === 2">
          <div class="setting-theme-item" v-for="(item, index) in themeList" :key="index" @click="setTheme(index)">
            <div class="preview" :style="{background: item.style.body.background}" :class="{noBoder: item.style.body.background !== '#fff'}"></div>
            <div class="text" :class="{selected: index === defaultTheme}">{{item.name}}</div>
          </div>
        </div>
        <div class="setting-font-size" v-else-if="showTag === 3">
          <div class="preview" :style="{fontSize: fontSizeList[0].fontSize + 'px'}">A</div>
          <div class="select">
            <div class="select-wrapper" v-for="(item, index) in fontSizeList" :key="index" @click="setFontSize(item.fontSize)">
              <div class="line"></div>
              <div class="point-wrapper" v-if="item.fontSize === defaultFontSize">
                <div class="point">
